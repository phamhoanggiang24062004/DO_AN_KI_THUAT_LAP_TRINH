...
void Print_MarkBoard_1Class(List_MarkBoard lb, List_Class lc, string name)
{
	...
	else
	{
		...
		while (St != NULL)
		{
			...
			{
				GPA = GPA / cnt;
				cout << "|" << right << setw(10) << "|" << right << setw(33) << "|" <<left<<setw(2)<<" "<< left << setw(30) << "DIEM GPA:" << left << setw(3) << "|" << right << setw(7) <<setprecision(2)<< GPA << right << setw(19) << "|" << endl;
		...
		cout << "Diem trung binh GPA cua lop " << name << " la: " << setprecision(2)<< (GPA_overall) / number_stu << endl;
	}
}
void Print_MarkBoard_1Student(List_MarkBoard lb, string ID)
{
	...
	if (GPA != 0 && tinchi != 0)
	{
		cout << "Diem GPA:   " << setprecision(2)<<(GPA / tinchi) << endl;
	}
}
