...
void Register(int choose, HeThong& HT)
{
	if (choose == 1)
	{
		...
		while (true)
		{
			...
			while (true)
			{
				Node_Register* temp = HT.REGIS_TEA.pHead;
				bool check = true;
				while (temp != NULL)
				{
					if (temp->data.pass.compare(pass) == 0)
					{
						check = false;
						break;
					}
				...
				if (check == false)
				{
					cout << "Mat khau nay da duoc su dung. Vui long nhap lai mat khau moi: ";
					cin >> pass;
				}
				else
				{
	...
	else if (choose == 2)
	{
		...
		while (true)
		{
			...
			{
				bool check = true;
				Node_Register* temp = HT.REGIS_STU.pHead;
				while (temp != NULL)
				{
					if (temp->data.pass.compare(pass) == 0)
					{
